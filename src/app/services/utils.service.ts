...
export class UtilsService {
  ...
  getColorPersonNode(value: number, maxValue: number, minValue: number, opacity: number) {
    ...
    // Rounded to avoid error parsing decimals numbers while rendering
    const newNumColor = Math.round((200 / parts) * (value - minValue));
    // rgb(255, 200, 0)
    // rgb(255, 0, 0)
    return `rgb(255, ${200 - (isNaN(newNumColor) ? 0 : newNumColor)}, 0, ${opacity})`;
  }

  // Could bring a value of NaN when max and min value are 0.
